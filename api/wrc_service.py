...
async def fetch_wrc_championship_standings(year: int) -> Optional[ChampionshipStandings]:
    ...
    try:
        async with httpx.AsyncClient() as client:
            seasons_res = await client.get(f"{WRC_API_URL}/seasons.json")
            ...
            season_detail_res = await client.get(f"{WRC_API_URL}/season-detail.json?seasonId={season_id}")
            season_detail_data = season_detail_res.json()
            ...
            champ_detail_res = await client.get(f"{WRC_API_URL}/championship-detail.json?championshipId={championship_id}&seasonId={season_id}")
            champ_detail_data = champ_detail_res.json()
            entry_map = {entry['championshipEntryId']: entry for entry in champ_detail_data.get('championshipEntries', [])}

            results_res = await client.get(f"{WRC_API_URL}/championship-overall-results.json?championshipId={championship_id}&seasonId={season_id}")
